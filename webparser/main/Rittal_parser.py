...
def parce (a,b):
    file = os.path.dirname(os.path.realpath(__file__)) + '\static\main\price\\rittal_price.xlsx'
    ...
    for k in range(a, b+1):
        ...
        try:
            ...
            req = Request(site, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.107 Safari/537.36'})
        ...
        for i in range(len(item_left_desc)):
            item_description += "<li>" + item_left_desc[i].getText() + " " + item_right_desc[i].getText().replace("\t","").replace("\n","") + "</li>" + "\n"
